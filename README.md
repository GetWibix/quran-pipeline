...
| **🎬 Shorts 30-59 ثانية** | متوافق مع تحديث YouTube الجديد (حتى 3 دقائق) |
| **📖 قراءة تسلسلية** | يتقدم عبر القرآن كامل (6236 آية) بدون تكرار |
| **🎙️多位 قراء** | مشاري العفاسي، عبدالباسط، الغامدي، المعيقلي |
| **📦 Docker** | جاهز للتشغيل الفوري على أي سيرفر |
| **🔄 طابور مهام** | BullMQ + Redis لإدارة العمليات بدون ازدواجية |
| **📊 متتبع Quota** | يحترم حدود YouTube API ولا يتجاوزها |
| **📱 إشعارات Telegram** | تقارير يومية عن النجاح والفشل |
| **🧹 تنظيف تلقائي** | يحذف الملفات المؤقتة لتوفير المساحة |
| **💾 استهلاك خفيف** | يعمل على VPS بذاكرة 1-2GB RAM |
...
└─────────────────┘     └──────────────────┘     └────────┬─────────┘
                    ...
                    ▼                                       ▼                      ▼
        ┌──────────────────────┐              ┌──────────────────────┐  ┌──────────────────────┐
        │  Content Pipeline    │              │  Decision Agent      │  │  YouTube Publisher   │
        │  ├ AI Verse Selector │              │  ├ Gemini/GPT Title  │  │  (quota-aware)       │
        │  ├ Verse Fetch       │              │  ├ Description + فوائد│  └──────────────────────┘
        │  ├ Audio Download    │              │  └ Scheduling         │
        │  ├ Visual Compose    │              └──────────────────────┘
        │  └ Video Render      │                                         ┌──────────────────────┐
        └──────────────────────┘                                         │  Notifier (Telegram) │
                                                                          └──────────────────────┘
...
│   │   ├── 📄 decisionAgent.ts     # AI للعنوان والوصف
│   │   ├── 📄 youtubePublisher.ts  # رفع ليوتيوب
│   │   ├── 📄 quotaTracker.ts      # متتبع الحصة
│   │   └── 📄 notifier.ts          # إشعارات Telegram
│   ├── 📁 queue/
│   │   ├── 📄 queue.ts             # BullMQ Queue
│   │   └── 📄 worker.ts            # معالج المهام
│   ├── 📁 scripts/
